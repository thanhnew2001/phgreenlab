...
export default function Devicedb() {
    ...
    useEffect(async () => {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const deviceresponse = await fetch(`https://thegreenlab.xyz/Datums/StatisticData?StartDate=${stDate}&EndDate=${EndDate}`, {
                method: 'GET',
                headers: { 'Authorization': 'Basic aGllbkBnbWFpbC5jb206MTIz' }
                    ...
                    {bigdata.map(b => {
                        return (
                            <tr >
                                ...
                                <td type="datetime">{moment(b.device.StartDate).format("DD/MM/YYYY, HH:mm")}</td>
                                <td type="datetime">{moment(b.device.EndDate).format("DD/MM/YYYY, HH:mm")}</td>
